...
function BusinessItem({business,showDir=false}) {
    ...
    const src = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photoreference=${photo_ref}&key=${GOOGLE_API_KEY}`;
    useEffect(()=>{
      calculateDistance(
        ...
        />
         <h2 className='text-[13px] font-bold mt-1 line-clamp-1'>{business.name}</h2>
          ...
          {showDir?  <div className='border-t-[1px] p-1 mt-1'>
              ...
              border-blue-500
              hover:text-white
              hover:bg-blue-500 text-center' onClick={()=>onDirectionClick()}  >Trazar una direccion</span></h2>
            </div>:null}
    </div>
  )
}
